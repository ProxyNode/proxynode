...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; prx-qt frontend will not be built])
    fi
...
dnl This must be called before any other BITCOIN_QT* mprxos to ensure that
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build prx-qt GUI (default=auto, qt5 tried first)])],
    [
...
dnl All mprxos below are internal and should _not_ be used from the main
dnl configure.ac.
dnl ----
